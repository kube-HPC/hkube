...
describe('deploymentCreator', () => {
    ...
    describe('sidecars', () => {
        ...
        it('should apply sidecar if enabled', () => {
            const res = createDeploymentSpec({ algorithmName: 'myalgo1', options: { kubernetes: {} }, clusterOptions: { "my-sidecarSidecarEnabled": true } });
        ...
        it('should not apply sidecar if no sidecar configmap', () => {
            const res = createDeploymentSpec({ algorithmName: 'myalgo1', options: { kubernetes: {} }, clusterOptions: { "no-sidecarSidecarEnabled": true } });
            expect(res.spec.template.spec.containers).to.have.lengthOf(1)
        });
    })
});
