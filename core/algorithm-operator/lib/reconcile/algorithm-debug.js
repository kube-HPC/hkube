...
const reconcile = async ({ services, debugAlgorithms, clusterOptions } = {}) => {
    ...
    const added = debugAlgorithms.filter(a => !normServices.find(d => `${d.algorithmName}-debug` === a.name));
    const removed = normServices.filter(d => !debugAlgorithms.find(a => `${d.algorithmName}-debug` === a.name));
