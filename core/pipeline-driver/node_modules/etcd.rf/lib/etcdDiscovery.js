...
class EtcdDiscovery extends EventEmitter {
    ...
    async init(options) {
        ...
        if (initSchemaConfig.valid) {
            ...
            throw new Error(etcdConfig.errorDescription)
        }
    }
    /**
    ...
    async set(options) {
        ...
        if (setSchemaInstance.valid) {
            ...
            let {
                ...
                instanceId,
                serviceName
            } = setSchemaInstance.instance;
            let internalServiceName = serviceName ? serviceName : this._serviceName
            let _path = path.join('/', PREFIX.SERVICES, internalServiceName, instanceId);
            this._data = data;
            try {
    ...
    async get(options) {
        return new Promise((res, rej) => {
            ...
            if (getSchemaInstance.valid) {
                let { serviceName, instanceId, index, etcdOptions } = getSchemaInstance.instance;
                let internalServiceName = serviceName ? serviceName : this._serviceName
                let _path = path.join('/', PREFIX.SERVICES, internalServiceName, instanceId);
