...
const _processAllRequests = (
    ...
    for (let r of normRequests) {// eslint-disable-line
        ...
        const { workerEnv, algorithmEnv, nodeSelector, entryPoint, options: algorithmOptions, reservedMemory, mounts, env } = algorithmTemplate;
...
const _algorithmsToMap = (requests) => {
...
/**
 * This method iterate all requests and search for algorithms with `minRequisiteAmount`.
 * If such algorithm found, we take the diff (minRequisiteAmount - running) and push it to
...
const createWindow = (algorithmTemplates, normRequests, idleWorkers, activeWorkers, pausedWorkers, pendingWorkers) => {
    ...
    if (minRequisiteAlgorithms.length > 0) {
        newRequests = [];
        const visit = {};
        const runningWorkers = [...idleWorkers, ...activeWorkers, ...pausedWorkers, ...pendingWorkers];
        const runningMap = _algorithmsToMap(runningWorkers);
        ...
        normRequests.forEach(r => {
            ...
            if (minRequisiteAmount && !visit[algorithmName]) {
                ...
                const running = runningMap[algorithmName] || 0;
