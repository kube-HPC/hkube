...
describe('Kubernetes API', () => {
    ...
    it('should log missing CRD warning only once', async () => {
        ...
        const logSpy = sinon.spy(log1, 'warning');
    ...
    it('should get all limit range data for containers', async () => {
        const res = await instance.getContainerDefaultResources();
        expect(res).to.be.an('object').to.have.keys('cpu', 'memory');
        expect(res.cpu).to.have.keys('defaultRequest', 'defaultLimits');
        expect(res.memory).to.have.keys('defaultRequest', 'defaultLimits');
    });

    it('should log limitrange warning only once', async () => {
        const log1 = Logger.GetLogFromContainer();

        const logSpy = sinon.spy(log1, 'warning');

        await instance.getContainerDefaultResources();
        await instance.getContainerDefaultResources();
        const matchingLogs = logSpy.getCalls().filter(call =>
            call.args[0].includes('Multiple LimitRanges with type=Container found') && call.args[0].includes('Taking the first one')
        );
