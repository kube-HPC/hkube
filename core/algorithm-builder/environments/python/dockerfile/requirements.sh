...
if [ ! -z ${PACKAGES_REGISTRY} ]; then
     ...
     pip install --trusted-host "${PACKAGES_REGISTRY_HOST}" --trusted-host "artifacts-registry" --index-url "http://artifacts-registry:8081/hkube/artifacts-registry/repository/python/simple" --extra-index-url "${PACKAGES_REGISTRY}"  -r /hkube/algorithm-runner/requirements.txt
else
