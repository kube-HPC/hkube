...
describe('DataSources', () => {
    before(() => {
        ...
        const serviceURI = `${protocol}://${host}:${port}/${prefix}`;
        ...
        nock(serviceURI)
            .persist()
            .get(`/validate?${qs3}`)
            .reply(400, { error: { code: 400, message: 'id non-exist Not Found' } });
        nock(serviceURI)
            .persist()
            .get(`/validate?${qs4}`)
            .reply(400, { error: { code: 400, message: 'snapshotName non-exist Not Found' } });
        nock(serviceURI)
            .persist()
            .get(`/validate?${qs5}`)
            .reply(400, { error: { code: 400, message: 'dataSource non-exist Not Found' } });
    });
    describe('/exec/raw', () => {
        it('should throw invalid kind', async () => {
            ...
            expect(response.body.error.message).to.equal('data.nodes[0].kind should be equal to one of the allowed values (algorithm,pipeline,debug,dataSource,gateway,output,hyperparamsTuner)');
        });
