...
const routes = (options) => {
  ...
  router.post("/graphByStreamingFlows", async (req, res) => {
    const { jobId, gateways } = await Execution.graphByStreamingFlows(req.body);
    res.json({ jobId, gateways });
  });
  router.post("/stop", async (req, res) => {
    ...
    const jobsToStop = searchResponse.hits.filter(
      (j) => j.status.status === "active" || j.status.status === "pending"
    );
  ...
  router.get("/flowInput/:jobId?", async (req, res) => {
    ...
    if (data) {
      if (req.query.download) {
        res.setHeader(
          "Content-Disposition",
          'attachment;filename="flowInput.json"'
        );
      }
      res.json(data);
    } else {
