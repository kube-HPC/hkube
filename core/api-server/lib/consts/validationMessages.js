module.exports = {
    PIPELINE_NAME_FORMAT: 'pipeline name must contain only alphanumeric, dash, dot or underscore',
    ALGORITHM_NAME_FORMAT: 'algorithm name must contain only lower-case alphanumeric, dash or dot',
    ALGORITHM_IMAGE_FORMAT: 'algorithm image must contain only lower-case alphanumeric, dash or dot',
    BOARD_NAME_FORMAT: 'board name must consist of lower case alphanumeric characters, dash or dot'
};
