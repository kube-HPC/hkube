...
class ExecutionService {
  ...
  async runCaching(options) {
    ...
    const pipeline = await cachingService.exec({
      jobId: options.jobId,
    ...
    if (debugNode) {
      ...
      debugOverrides = debugOverrides.filter((v) =>
        restPipeline.nodes.find((n) => n.nodeName === v)
      );
  ...
  async rerun(options) {
    ...
    const { jobId } = options;
    const job = await stateManager.getJob({
      jobId,
      fields: { types: "pipeline.types", userPipeline: true },
    });
    if (!job) {
    ...
    return this._runPipeline({
      pipeline: job.userPipeline,
      types,
      options: { validateNodes: false },
    });
  }

  async graphByStreamingFlows(options) {
    validator.executions.validateRerun(options);
    const { jobId } = options;
    const job = await stateManager.getJob({
      jobId,
    ...
    const types = [...job.types, pipelineTypes.RERUN];
    return this._graphFlowStreamingFlows({
      pipeline: job.userPipeline,
      types,
      options: { validateNodes: false },
    });
  ...
  async _runStored(options) {
    const { pipeline, jobId, rootJobId, parentSpan, types, mergeFlowInput } =
      options;
    const storedPipeline = await stateManager.getPipeline({
      name: pipeline.name,
    });
    if (!storedPipeline) {
  ...
  async _graphFlowStreamingFlows(payload) {
    ...
    const span = tracer.startSpan({
      name: "run pipeline",
      tags: { jobId, name: extendedPipeline.name },
      parent: parentSpan,
    });
    try {
      validator.pipelines.validatePipelineNodes(extendedPipeline);
      const maxExceeded =
        await validator.executions.validateConcurrentPipelines(
          extendedPipeline
        );
      extendedPipeline = await pipelineCreator.buildPipelineOfPipelines(
        extendedPipeline
      );
      extendedPipeline = await pipelineCreator.updateDebug(extendedPipeline);
      extendedPipeline = await pipelineCreator.updateOutput(
        extendedPipeline,
        jobId
      );
      extendedPipeline = await pipelineCreator.updateOptimize(
        extendedPipeline,
        jobId
      );
      const algorithms = await validator.algorithms.validateAlgorithmExists(
        extendedPipeline
      );
      extendedPipeline = await pipelineCreator.buildStreamingFlow(
        extendedPipeline,
        jobId,
        algorithms
      );
      ...
      validator.executions.validatePipeline(
        {
          ...extendedPipeline,
          flowInput: extendedPipeline.flowInput || flowInput,
        },
        { validateNodes: shouldValidateNodes }
      );
      ...
      if (
        flowInput &&
        Object.keys(flowInput).length &&
        !pipeFlowInputMetadata
      ) {
        const metadata = parser.replaceFlowInput({
          ...extendedPipeline,
          flowInput,
        });
        const storageInfo = await storageManager.hkube.put(
          { jobId, taskId: jobId, data: flowInput },
          tracer.startSpan.bind(tracer, {
            name: "storage-put-input",
            parent: span.context(),
          })
        );
      ...
      const pipelineObject = {
        ...
        maxExceeded,
        rootJobId,
        flowInputMetadata: pipeFlowInputMetadata,
        startTime: Date.now(),
        lastRunResult,
        types: pipeTypes,
      };
      const statusObject = {
        timestamp: Date.now(),
        pipeline: extendedPipeline.name,
        status: pipelineStatuses.PENDING,
        level: levels.INFO.name,
      };
      await storageManager.hkubeIndex.put(
        { jobId },
        tracer.startSpan.bind(tracer, {
          name: "storage-put-index",
          parent: span.context(),
        })
      );
  ...
  async getPipeline(options) {
    ...
    if (!pipeline.flowInput && pipeline.flowInputMetadata?.storageInfo) {
      pipeline.flowInput = await storageManager.storage.get(
        pipeline.flowInputMetadata?.storageInfo
      );
    }
    return pipeline;
  }
  ...
  async getJobResult(options) {
    ...
    const job = await stateManager.getJob({
      jobId,
      fields: { status: true, result: true },
    });
    ...
    if (!result) {
      throw new InvalidDataError(
        `unable to get results for pipeline ${status.pipeline} because its in ${status.status} status`
      );
    }
    const response = await stateManager.getResultFromStorage({
      jobId,
      ...result,
    });
    return response;
  ...
  async stopJob(options) {
    ...
    if (jobId) {
      ...
      const job = await stateManager.getJob({
        jobId,
        fields: { status: true, result: true, pipeline: true },
      });
      ...
      if (result) {
        throw new InvalidDataError(
          `unable to stop pipeline ${pipeline} because it's in ${status} status`
        );
      }
    } else if (options.job) {
      ({ jobId } = options.job || {});
  ...
  async pauseJob(options) {
    ...
    if (result) {
      throw new InvalidDataError(
        `unable to pause pipeline ${status.pipeline} because its in ${status.status} status`
      );
    }
    const statusObject = {
      jobId,
      status: pipelineStatuses.PAUSED,
      level: levels.INFO.name,
    };
    await stateManager.updateJobStatus(statusObject);
  }
  ...
  async resumeJob(options) {
    ...
    if (!this.isPausedState(jobStatus.status)) {
      throw new InvalidDataError(
        `unable to resume pipeline ${jobStatus.pipeline} because its in ${jobStatus.status} status`
      );
    }
    const statusObject = {
      jobId,
      status: pipelineStatuses.RESUMED,
      level: levels.INFO.name,
    };
    await stateManager.updateJobStatus(statusObject);
  ...
  async getFlowInputByJobId(jobId) {
    ...
    if (pipeline?.flowInput) {
      ...
      flowInput = await storageManager.storage.get(
        pipeline.flowInputMetadata.storageInfo
      );
    }
    return flowInput;
